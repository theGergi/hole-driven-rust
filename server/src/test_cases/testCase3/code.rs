...
fn main() {
	...
	let s_imm_borrow: &string = &s; // Immutable borrow created

	immutable_borrow(??); // expects 's_imm_borrow', '&s' - can create another imm borrow or use existing
		
	let s_imm_borrow_2: &string = ??; // expects 's_imm_borrow', '&s' - still can borrow imm

	immutable_borrow(??); // expects 's_imm_borrow', '&s' - multiple imm borrows ok
		
	let s_mut_borrow: &mut string = ??; // expects '&mut s' - can create mut borrow since no other borrows active

	mutable_borrow(??);   // expects '&mut s' - mut borrow still active

	{
		let s_mut_borrow_2: &mut string = ??; // expects [] - s_imm_borrow used later, can't create mut borrow
	...
	let s_mut_borrow_3: &mut string = ??; // expects '&mut s' - imm borrow scope ended, can borrow mut
